...
async function generateQuestion(q){
  ...
  try{
    ...
    if(payload.uso) somaUso(payload.uso);
    /* v18.25 — VALIDAÇÃO INDEPENDENTE DA FONTE (backend v74.21). Em Linguagens e
...
function auditaQuestaoLocal(q){
  ...
  if(q.validacaoFonte && typeof q.validacaoFonte === "object"){
    ...
    if(v.libera === true){
      info("Fonte validada pelo agente validador · nível " + String(v.nivel || "?") + " · suporte " + String(v.suporte || "?") +
           " · confiança " + String(v.confianca || "?") + (v.fonteAberta ? " · página aberta" : " · página não aberta (conferida pelos resultados da busca)") +
