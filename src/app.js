...
function auditaQuestaoLocal(q){
  ...
  if(cal){
    ...
    /* v18.23 — O AVISO PASSOU A SER O p90 REAL, NÃO O p75.
       Medição das provas oficiais 2015-2025 (326 questões completas, 1.700
       alternativas, extraídas dos PDFs do INEP): a média das cinco fica acima
       do p75 em 20% das questões REAIS — um p75 deixa 25% acima por definição,
       então o aviso estava reprovando uma em cada cinco questões do próprio
       ENEM. O p90 medido da média é ~1,3 vez o p75 (Linguagens 88/67,
       Humanas 68/47, Natureza 60/43), e é esse o teto agora: o aviso passa a
       significar "mais longa que 90% das questões reais". Texto-base e comando
       seguem no p75 — não medi o p90 deles e não mexo no que não medi. */
    const tetoItem = Math.round(cal.item[1] * 1.3);
  ...
  if(confAud.letra && txt(confAud.letra)){
    ...
    /* v18.23 — 1,30 REPROVAVA O PRÓPRIO ENEM. Medido nas provas oficiais
       2015-2025, com o mesmo filtro deste ramo (menor > 40 caracteres): a razão
       maior/menor tem p50 1,24 · p75 1,37 · p90 1,47 · p95 1,59, e 37,3% das
       questões REAIS passam de 1,30. As questões que este app gera já são mais
       uniformes que as reais (p90 de 1,30 em História a 1,36 em Artes). O
       limiar vai para 1,50, entre o p90 e o p95 do exame: avisa no que o ENEM
       de fato considera desigual, e cala no que é normal. O prompt continua
       pedindo 1,25 — alvo apertado, alarme largo. */
    } else if(menor > 40 && maior > 1.50 * menor){
      info(`Alternativas com extensões desiguais (de ${menor} a ${maior} caracteres, razão ${(maior/menor).toFixed(2)}) — nas provas reais do ENEM o p90 é 1,47. O Guia do Inep pede paridade técnica entre as cinco.`);
    }
  }
