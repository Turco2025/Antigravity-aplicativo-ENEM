...
function temasEvitarPara(q){
  ...
  state.questions.forEach(o => {
    ...
    const mesmoGrupo = comRecorte && grupoDeTema(o) === grupoProprio && !!o.recorte;
    ...
    const digitado = mesmoGrupo ? "" : (o.tema || "").trim();
...
async function planejaRecortesPorTema(){
  ...
  for(const g of grupos.values()){
    ...
    try{
      ...
      g.qs.forEach((q, i) => {
        let r = i < payload.recortes.length ? Object.assign({}, payload.recortes[i]) : null;
        // v18: com lista do lote, o recorte tem de ficar no conteúdo desta questão;
        // se o planejador (ou um backend antigo) trocou os conteúdos, o recorte
        // inteiro sai — o tema da questão e o domínio reservado bastam.
        if(r && g.lista && !recorteRespeitaItem(r, q.tema, g.qs.map(o => o.tema))){
          console.warn(`[tema] recorte ${state.questions.indexOf(q) + 1}: fora do conteúdo distribuído ("${q.tema}") — descartado: "${String(r.conteudo || "").slice(0, 120)}"`);
          r = null;
        }
        if(r && r.contexto){
